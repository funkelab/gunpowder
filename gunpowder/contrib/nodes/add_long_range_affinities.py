...
class AddLongRangeAffinities(BatchFilter):
    ...
    def __init__(
            ...
            array_type_1,
            array_type_2,
            affinity_array_type_1,
            affinity_array_type_2):

        self.array_type_1 = array_type_1
        self.array_type_2 = array_type_2
        self.affinity_array_type_1 = affinity_array_type_1
        self.affinity_array_type_2 = affinity_array_type_2
        self.affinity_vectors = affinity_vectors

    def setup(self):
        assert self.array_type_1 in self.spec, "Upstream does not provide %s needed by \
        AddGtAffinities"%self.array_type_1
        assert self.array_type_2 in self.spec, "Upstream does not provide %s needed by \
        AddGtAffinities"%self.array_type_2

        voxel_size = self.spec[self.array_type_1].voxel_size
        ...
        spec = self.spec[self.array_type_1].copy()
        # if spec.roi is not None:

        self.provides(self.affinity_array_type_1, spec)
        self.provides(self.affinity_array_type_2, spec)
        self.enable_autoskip()

    def prepare(self, request):

        array_1_roi = request[self.array_type_1].roi
        logger.debug("downstream %s request: "%self.array_type_1 + str(array_1_roi))

        array_2_roi = request[self.array_type_2].roi
        logger.debug("downstream %s request: "%self.array_type_2 + str(array_2_roi))
        ...
        request[self.array_type_1].roi = array_1_roi
        request[self.array_type_2].roi = array_2_roi

        logger.debug("upstream %s request: "%self.array_type_1 + str(array_1_roi))
        logger.debug("upstream %s request: "%self.array_type_2 + str(array_2_roi))

        # pdb.set_trace()

    def process(self, batch, request):

        full_vol1 = batch.arrays[self.array_type_1]
        full_vol2 = batch.arrays[self.array_type_2]
        ...
        request_vol = request[self.affinity_array_type_1]
        ...
        batch.arrays[self.affinity_array_type_1] = Array(affinity_map,
            spec=request[self.affinity_array_type_1].copy())

        batch.arrays[self.affinity_array_type_1].attrs['affinity_vectors'] =\
         self.affinity_vectors

        # Calculate affinities 2: from vol1 onto vol2

        # Initialize affinity map
        request_vol = request[self.affinity_array_type_2]
        affinity_map = np.zeros(
            (len(self.affinity_vectors),) +
            tuple(request_vol.roi.get_shape()/request_vol.voxel_size), dtype=full_vol1.spec.dtype)
        ...
        batch.arrays[self.affinity_array_type_2] = Array(affinity_map,
            spec=request[self.affinity_array_type_2].copy())

        batch.arrays[self.affinity_array_type_2].attrs['affinity_vectors'] =\
         self.affinity_vectors



        # Crop all other requests
        for array_type, array in request.array_specs.items():
            batch.arrays[array_type] = batch.arrays[array_type].crop(array.roi)
