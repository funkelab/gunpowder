...
class IterateLocations(BatchFilter):
    ...
    def setup(self):
        ...
        logger.info("Requesting all %s points in roi %s",
                    self.graph, query_spec.roi)
    ...
    def _get_next_shift(self, center_shift, voxel_size):
        ...
        if self.choose_randomly:
            ...
            with IterateLocations.__global_index.get_lock():
                ...
                if IterateLocations.__global_index.value ==\
                        len(self.coordinates) - 1:
                    logger.info(
                        "After this request, all points have been visited")
                ...
                self.local_index = IterateLocations.__global_index.value %\
                    len(self.coordinates)
        next_shift = (Coordinate(self.coordinates[self.local_index])
                      - center_shift)
    ...
    def __check_shift(self, request):
        for key, spec in request.items():
            ...
            if not provided_roi.contains(shifted_roi):
                logger.warning(("Provided roi %s for key %s does notcontain"
                                " shifted roi %s"),
