...
def train(max_iteration):
    ...
    request.add_volume_request(ArrayTypes.GT_BM_PRESYN, shape_output)
    request.add_volume_request(ArrayTypes.GT_MASK_EXCLUSIVEZONE_PRESYN, shape_output)

    # define networks input, output and groundtruth names and ArrayTypes
    net_inputs =  [
                    net_input(volume_name='data', volume_type=ArrayTypes.RAW)
                  ]

    net_outputs = [
                    net_output(volume_name='bm_presyn_pred', volume_type=ArrayTypes.PRED_BM_PRESYN,
                               gt_name='bm_presyn_label', loss_volume_type=ArrayTypes.LOSS_GRADIENT_PRESYN),
                  ]
    net_gts =     [
                    net_gt(volume_name='bm_presyn_label', volume_type=ArrayTypes.GT_BM_PRESYN,
                           scale_name='bm_presyn_scale', mask_volume_type=ArrayTypes.GT_MASK_EXCLUSIVEZONE_PRESYN),
                  ]
