...
class AddDummyPredictions(BatchFilter):
    ...
    def process(self, batch, request):
        batch.volumes[VolumeType.PRED_AFFINITIES] = np.ones(
                request.volumes[VolumeType.PRED_AFFINITIES], dtype=np.float32
        )*0.5
...
def train():
    ...
    request.add_volume_request(VolumeType.PRED_AFFINITIES, (84,268,268))
