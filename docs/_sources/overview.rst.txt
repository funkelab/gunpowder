...
In the following we will see how train a neural network directly in this
pipeline. For the example, we will train a 2D U-Net (``model``) and the binary
...
.. admonition:: Training Preliminaries: Create Model, Loss, and Optimizer
  ...
  the two folloing levels. The output of the U-Net is further passed through a
...
.. admonition:: Further Example: Prediction in Large nD Arrays
  ...
  fashoin over the whole input range--the only difference is that :class:`Scan`
