    ...
    def test_output(self):
        ...
        pipeline = (
            ...
            PrepareMalis(
                ...
                ignore_volume_type=ArrayTypes.GT_IGNORE)
        )
        ...
        with build(pipeline):
            ...
            self.assertTrue(ArrayTypes.MALIS_COMP_LABEL not in batch.volumes)
        ...
        with build(pipeline):
            ...
            # test if volume added to batch
            self.assertTrue(ArrayTypes.MALIS_COMP_LABEL in batch.volumes)

            # test if gt_ignore considered for gt_neg_pass ([0, ...]) and not for gt_pos_pass ([1, ...])
            ignored_locations = np.where(batch.volumes[ArrayTypes.GT_IGNORE].data == 0)
            # gt_neg_pass
            self.assertTrue((batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[0,...][ignored_locations] == 3).all())
            self.assertFalse((np.array_equal(batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[0, ...],
                                            batch.volumes[ArrayTypes.GT_LABELS].data)))
            # gt_pos_pass
            self.assertFalse((batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[1,...][ignored_locations] == 3).all())
            self.assertTrue((np.array_equal(batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[1, ...],
                                            batch.volumes[ArrayTypes.GT_LABELS].data)))
        ...
        with build(pipeline):
            ...
            # test if gt_ignore considered for gt_neg_pass ([0, ;;;]) and not for gt_pos_pass ([1, ...])
            # gt_neg_pass
            self.assertTrue((np.array_equal(batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[0, ...],
                                            batch.volumes[ArrayTypes.GT_LABELS].data)))
            # gt_pos_pass
            self.assertTrue((np.array_equal(batch.volumes[ArrayTypes.MALIS_COMP_LABEL].data[1, ...],
                                            batch.volumes[ArrayTypes.GT_LABELS].data)))
