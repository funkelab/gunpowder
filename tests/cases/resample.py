...
from gunpowder import ArrayKey, ArraySpec, Roi, Coordinate, Batch, BatchRequest, Array, MergeProvider, Resample, build
import numpy as np
...
def test_up_and_downsample():
    ...
    raw_source = ArraySource(raw_key, Array(array, ArraySpec(Roi((0,0,0),(1000,1000,1000)), Coordinate(4,4,4))))
    gt_source = ArraySource(gt_key, Array(array, ArraySpec(Roi((0,0,0),(1000,1000,1000)), Coordinate(4,4,4))))
    ...
    pipeline = (
            ...
            Resample(gt_key, Coordinate((2,2,2)), gt_resampled_key, interp_order=0) #Test upsampling, without interpolation
    )
    ...
    for (array_key, array) in batch.arrays.items():
        ...
        if array_key in [raw_key, gt_key]:
            ...
            assert array.data[0,0,0,0] == 31, f'RAW_RESAMPLED[0,0,0]: {array.data[0,0,0]} does not equal expected: 31'
            assert array.data[0,1,0,0] == 33, f'RAW_RESAMPLED[1,0,0]: {array.data[1,0,0]} does not equal expected: 33'
            ...
            assert array.data[0,0,0,0] == 3, f'GT_LABELS_RESAMPLED[0,0,0]: {array.data[0,0,0]} does not equal expected: 0'
            assert array.data[0,1,0,0] == 3, f'GT_LABELS_RESAMPLED[1,0,0]: {array.data[1,0,0]} does not equal expected: 0'
            assert array.data[0,2,0,0] == 4, f'GT_LABELS_RESAMPLED[2,0,0]: {array.data[2,0,0]} does not equal expected: 1'
            assert array.data[0,3,0,0] == 4, f'GT_LABELS_RESAMPLED[3,0,0]: {array.data[3,0,0]} does not equal expected: 1'
