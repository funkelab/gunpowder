...
class TestPlaceholderRequest(ProviderTest):
    def test_without_placeholder(self):
        ...
        with build(pipeline):
            for i in range(100):
                ...
                points_a = batch_a[test_points].data
                points_b = batch_b[test_points].data

                self.assertIn(test_labels, batch_b)

                for a, b in zip(points_a.values(), points_b.values()):
                    equality.append(all(np.isclose(a.location, b.location)))
                    break
        self.assertAlmostEqual(0.25, np.mean(equality), delta=0.1)
    ...
    def test_placeholder(self):
        ...
        self.assertEqual(1, np.mean(equality))
